...
:::tip 若要查看 superfile 檔案的路徑位置，請使用 `spf pl` 指令。:::
...
:::note 你需要確認 powershell 允許執行 script。如果出現像 `running scripts is disabled on this system` 這樣的錯誤，就需要允許它。
...
:::caution 即使已啟用額外欄位，如果 filepanel 尺寸太小，欄位仍會被隱藏。如果看不到欄位，請增加大小或調整 file_panel_name_percent 設定。:::
...
:::note除非 `nerdfont = true`，否則此設定會被忽略。`nerdfont` 和 `show_select_icons` 都必須為 `true`，checkbox icons 才會出現。:::
...
:::caution 請確認加入的字串寬度剛好是一個字元。若不要邊框，請使用 ' ' :::
